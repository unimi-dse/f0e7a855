...
prepare <- function() {
  ...
  airlinesafety::air_safety<-cbind(airlinesafety::air_safety,fa_rate) %>%
    mutate(safety = fa_rate<0.2)
  #Check the dimensions of the dataset
  return(airlinesafety::air_safety)
}
...
fa <- function(){
  ...
  airlinesafety::air_safety<-prepare() %>%
    mutate(safety = fa_rate<0.2)
  #Check the dimensions of the dataset
  dim(airlinesafety::air_safety)
}
...
plot1 <- function(x, y, tipe = 'bar', title) {
  ...
  b <- plot_ly(airlinesafety::air_safety, x = ~airlinesafety::air_safety$airline, y = ~airlinesafety::air_safety$fatal_accidents_85_99, type = 'bar', name = '1985-1999') %>%
    add_trace(y = ~airlinesafety::air_safety$fatal_accidents_00_14, name = '2000-2014') %>%
...
reg <- function(){
  ...
  reg_1<-lm(air_safety$fatal_accidents_00_14~air_safety$incidents_00_14, data=airlinesafety::air_safety)
  summary(reg_1)
  #Multiple Linear Regressions
  reg_2<-lm(air_safety$fatal_accidents_00_14~air_safety$incidents_00_14+air_safety$fatalities_00_14, data=airlinesafety::air_safety)
  summary(reg_2)
